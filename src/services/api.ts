...
if (!apiUrlEvoAi) {
  console.error(
    'VITE_API_AI_URL não está definido no ambiente. As chamadas para a API da Evo AI podem falhar.',
  );
}
...
apiInternal.interceptors.request.use(
  (config) => {
    console.log(
      `[apiInternal] Making request to: ${config.baseURL}${config.url}`,
    );
    ...
    if (isAuthEndpoint) {
      console.log(
        `[apiInternal Interceptor] Skipping auth headers for auth endpoint: ${config.url}`,
      );
    ...
    if (
      (isSharedAgentRequest || isSharedChatPage) &&
      localStorage.getItem('shared_agent_api_key')
    ) {
      ...
      console.log(
        `[apiInternal Interceptor] Using shared agent API key for: ${config.url}`,
      );
    } else {
      ...
      if (internalToken) {
        ...
        console.log(
          `[apiInternal Interceptor] Adding Internal token for: ${config.url}`,
        );
      } else {
        console.warn(
          `[apiInternal Interceptor] Internal token not found for ${config.url}. This may cause a 401.`,
        );
...
apiEvoAi.interceptors.request.use(
  (config) => {
    console.log(
      `[apiEvoAi] Making request to: ${config.baseURL}${config.url}`,
    );
    ...
    if (evoAiToken) {
      ...
      console.log(
        `[apiEvoAi Interceptor] Adding Evo AI token for: ${config.url}`,
      );
    } else {
      console.warn(
        `[apiEvoAi Interceptor] Evo AI token not found for ${config.url}. This may cause a 401.`,
      );
      // O 401 response interceptor lidará com isso
    }
...
const handleUnauthorizedResponse = async (error: any) => {
  ...
  if (
    ...
    if (!isAuthEndpoint && !isSharedChatPage) {
      console.warn(
        `[API Interceptor] Received 401 Unauthorized from ${error.config.baseURL}. Initiating logout.`,
      );
      ...
      console.log(
        `[API Interceptor] Received 401 Unauthorized from ${error.config.baseURL} for an auth or shared chat endpoint. Skipping logout.`,
      );
...
apiEvoAi.interceptors.response.use(
  (response) => response,
  handleUnauthorizedResponse,
);
...
export function post<T>(
  arg0: string,
  arg1: { id: string; client_id: string; agent_id: string },
) {
