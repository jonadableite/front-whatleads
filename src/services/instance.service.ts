...
export const calculateWarmupProgress = (instance: Instancia) => {
  // Se não tiver warmupStats ou não for um array, retorna 0
  if (
    !instance.warmupStats ||
    !Array.isArray(instance.warmupStats) ||
    instance.warmupStats.length === 0
  ) {
    return 0;
  }
  ...
  const progress =
    typeof warmupTimeInSeconds === "number" && warmupTimeInSeconds > 0
