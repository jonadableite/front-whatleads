...
const api = axios.create({
  baseURL: import.meta.env.VITE_API_URL || "https://filtro.whatlead.com.br",
  ...
  (config) => {
    ...
    if (!config.timeout) {
      if (
        ...
        (config.url?.includes("/start") || config.url?.includes("/resume"))
      ) {
        // Timeout muito maior para iniciar/retomar campanhas (operação demorada)
        config.timeout = 120000; // 2 minutos
      } else if (config.url?.includes("/groups/fetchAllGroups")) {
        ...
        // Timeout médio para outras operações de campanha
        config.timeout = 30000; // 30 segundos
      } else {
